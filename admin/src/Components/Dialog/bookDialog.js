...
import { CLOSE_BOOK_DIALOG } from "../../store/book/types.js";
import { createNewBOOK, editBOOK } from "../../store/book/action.js";
...
const BOOKDialog = (props) => {
  ...
  useEffect(() => {
    if (dialogData) {
      ...
      setBOOK(dialogData?.questions);
      setImagePath(`${baseURL}/${dialogData?.pdf_book}`);
    }
  }, [dialogData]);
  ...
  const handleSubmit = (e) => {
    ...
    if (!name) {
      const errors = {};
      if (!name) {
        errors.name = "BOOK name can't be a blank!";
      }
    ...
    formData?.append("pdf_book", imageData);
    ...
    if (mongoId) {
      props.editBOOK(formData, mongoId);
    } else {
  ...
  return (
    <Fragment>
      <Dialog
        ...
        maxWidth="sm"
      >
        <DialogTitle id="responsive-dialog-title">BOOK</DialogTitle>
        <IconButton
          style={{
            position: "absolute",
            right: 0,
                ...
                <div class="row">
                  <div class="col-md-12">
                    ...
                    <div class="form-group">
                      <label class="float-left">BOOK Name</label>
...
export default connect(null, { createNewBOOK, editBOOK })(BOOKDialog);
