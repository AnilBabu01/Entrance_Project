...
import { CLOSE_NOTESUNIT_DIALOG } from "../../store/units/types.js";
import { createNewNOTESUNIT, editNOTESUNIT } from "../../store/units/action.js";
...
const NOTESUNITDialog = (props) => {
  ...
  const {
    ...
    dialogData,
    NOTESUNIT: note,
  } = useSelector((state) => state.NOTESUNIT);
  ...
  const handleSubmit = (e) => {
    ...
    if (!name) {
      const errors = {};
      if (!name) {
        errors.name = "NOTESUNIT name can't be a blank!";
      }
    ...
    if (mongoId) {
      props.editNOTESUNIT(data, mongoId);
    } else {
      props.createNewNOTESUNIT(data);
    }
  };
  ...
  return (
    <Fragment>
      ...
      >
        <DialogTitle id="responsive-dialog-title">NOTESUNIT</DialogTitle>
        <IconButton
          style={{
            position: "absolute",
            right: 0,
                ...
                <div class="row">
                  <div class="col-md-12">
                    <div className="form-group">
                      <TextEditor
                        onChange={(content) => {
                          setNOTESUNIT(content);
                        }}
                        blogHtml={NOTESUNIT}
                        setBlogHtml={setNOTESUNIT}
                      />
                    </div>
                  </div>
                </div>
...
export default connect(null, { createNewNOTESUNIT, editNOTESUNIT })(
  NOTESUNITDialog
);
