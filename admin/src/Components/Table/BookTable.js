...
const BOOKtable = (props) => {
  ...
  const { BOOK, createDone, updateDone, deleteDone } = useSelector(
    (state) => state.Book
  );
  ...
  const blockUnblock = (data) => {
    props.blockUnblockBOOK(data.id);
  };

  const handleOpen = () => {
    dispatch({ type: OPEN_BOOK_DIALOG });
  };

  const handleEdit = (data) => {
    dispatch({ type: OPEN_BOOK_DIALOG, payload: data });
  };
  ...
  useEffect(() => {
    if (createDone) {
      setOpenSuccess(true);
      dispatch({ type: UNSET_CREATE_BOOK_DONE });
    }
  }, [createDone, dispatch]);
  ...
  useEffect(() => {
    if (updateDone) {
      ...
      dispatch({ type: UNSET_UPDATE_BOOK_DONE });
    }
  }, [updateDone, dispatch]);
  ...
  useEffect(() => {
    if (deleteDone) {
      ...
      dispatch({ type: UNSET_TEST_DELETE_BOOK_DONE });
    }
  }, [deleteDone, dispatch]);
    ...
    <Fragment>
      ...
      >
        <Alert onClose={handleCloseSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> BOOK add successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseUpdateSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> BOOK update successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseDeleteSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> BOOK delete successfully.
          </span>
        </Alert>
      </Snackbar>
