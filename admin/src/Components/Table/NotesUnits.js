...
//dialog
import NOTESUNITDialog from "../Dialog/unitsDialog";

//redux
import { useDispatch, connect, useSelector } from "react-redux";
import {
  getNOTESUNIT,
  blockUnblockNOTESUNIT,
  deleteTestcategory,
} from "../../store/units/action";
...
const NOTESUNITtable = (props) => {
  ...
  const { NOTESUNIT, createDone, updateDone, deleteDone } = useSelector(
    (state) => state.NOTESUNIT
  );
  ...
  const blockUnblock = (data) => {
    props.blockUnblockNOTESUNIT(data.id);
  };

  const handleOpen = () => {
    dispatch({ type: OPEN_NOTESUNIT_DIALOG });
  };

  const handleEdit = (data) => {
    dispatch({ type: OPEN_NOTESUNIT_DIALOG, payload: data });
  };
  ...
  useEffect(() => {
    if (createDone) {
      setOpenSuccess(true);
      dispatch({ type: UNSET_CREATE_NOTESUNIT_DONE });
    }
  }, [createDone, dispatch]);
  ...
  useEffect(() => {
    if (updateDone) {
      ...
      dispatch({ type: UNSET_UPDATE_NOTESUNIT_DONE });
    }
  }, [updateDone, dispatch]);
  ...
  useEffect(() => {
    if (deleteDone) {
      ...
      dispatch({ type: UNSET_TEST_DELETE_NOTESUNIT_DONE });
    }
  }, [deleteDone, dispatch]);
  ...
  const handleSearch = (e) => {
    ...
    if (value) {
      const filteredData = NOTESUNIT.filter((data) => {
        ...
        return (
          data?.unit?.toUpperCase().includes(value) ||
    ...
    <Fragment>
      ...
      >
        <Alert onClose={handleCloseSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> Unit add successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseUpdateSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> Unit update successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseDeleteSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> Unit delete successfully.
          </span>
        </Alert>
      </Snackbar>
