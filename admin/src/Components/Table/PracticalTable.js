...
//dialog
import PRACTICALDialog from "../Dialog/practicalDialog";

//redux
import { useDispatch, connect, useSelector } from "react-redux";
import {
  getPRACTICAL,
  blockUnblockPRACTICAL,
  deleteTestcategory,
} from "../../store/practical/action";
...
const PRACTICALtable = (props) => {
  ...
  const { PRACTICAL, createDone, updateDone, deleteDone } = useSelector(
    (state) => state.Practical
  );
  ...
  const blockUnblock = (data) => {
    props.blockUnblockPRACTICAL(data.id);
  };

  const handleOpen = () => {
    dispatch({ type: OPEN_PRACTICAL_DIALOG });
  };

  const handleEdit = (data) => {
    dispatch({ type: OPEN_PRACTICAL_DIALOG, payload: data });
  };
  ...
  useEffect(() => {
    if (createDone) {
      setOpenSuccess(true);
      dispatch({ type: UNSET_CREATE_PRACTICAL_DONE });
    }
  }, [createDone, dispatch]);
  ...
  useEffect(() => {
    if (updateDone) {
      ...
      dispatch({ type: UNSET_UPDATE_PRACTICAL_DONE });
    }
  }, [updateDone, dispatch]);
  ...
  useEffect(() => {
    if (deleteDone) {
      ...
      dispatch({ type: UNSET_TEST_DELETE_PRACTICAL_DONE });
    }
  }, [deleteDone, dispatch]);
    ...
    <Fragment>
      ...
      >
        <Alert onClose={handleCloseSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> PRACTICAL add successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseUpdateSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> PRACTICAL update successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseDeleteSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> PRACTICAL delete successfully.
          </span>
        </Alert>
      </Snackbar>
                            ...
                            return (
                              <tr key={index}>
                                ...
                                <td style={{ verticalAlign: "middle" }}>
                                  {data.pdf_practical ? (
                                    <a
                                      href={`${baseURL}/${data.pdf_practical}`}
                                      target="_blank"
