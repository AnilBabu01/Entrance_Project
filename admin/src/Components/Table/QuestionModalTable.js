...
//dialog
import QUESTIONMODALDialog from "../Dialog/QuestionModalDialog";

//redux
import { useDispatch, connect, useSelector } from "react-redux";
import {
  getQUESTIONMODAL,
  blockUnblockQUESTIONMODAL,
  deleteTestcategory,
} from "../../store/questionmodal/action";
...
} from "../../store/questionmodal/types";
...
const QUESTIONMODALtable = (props) => {
  ...
  const { QUESTIONMODAL, createDone, updateDone, deleteDone } = useSelector(
  ...
  useEffect(() => {
    setData(QUESTIONMODAL);
  }, [QUESTIONMODAL]);

  const blockUnblock = (data) => {
    props.blockUnblockQUESTIONMODAL(data.id);
  };

  const handleOpen = () => {
    dispatch({ type: OPEN_QUESTIONMODAL_DIALOG });
  };

  const handleEdit = (data) => {
    dispatch({ type: OPEN_QUESTIONMODAL_DIALOG, payload: data });
  };
  ...
  useEffect(() => {
    if (createDone) {
      setOpenSuccess(true);
      dispatch({ type: UNSET_CREATE_QUESTIONMODAL_DONE });
    }
  }, [createDone, dispatch]);
  ...
  useEffect(() => {
    if (updateDone) {
      ...
      dispatch({ type: UNSET_UPDATE_QUESTIONMODAL_DONE });
    }
  }, [updateDone, dispatch]);
  ...
  useEffect(() => {
    if (deleteDone) {
      ...
      dispatch({ type: UNSET_TEST_DELETE_QUESTIONMODAL_DONE });
    }
  }, [deleteDone, dispatch]);
    ...
    <Fragment>
      ...
      >
        <Alert onClose={handleCloseSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> QUESTIONMODAL add successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseUpdateSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> QUESTIONMODAL update successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseDeleteSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> QUESTIONMODAL delete successfully.
          </span>
        </Alert>
      </Snackbar>
