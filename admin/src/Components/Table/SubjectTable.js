...
const SUBJECTtable = (props) => {
  ...
  const { SUBJECT, createDone, updateDone, deleteDone } = useSelector(
    (state) => state.subject
  );
  ...
  const blockUnblock = (data) => {
    props.blockUnblockSUBJECT(data.id);
  };

  const handleOpen = () => {
    dispatch({ type: OPEN_SUBJECT_DIALOG });
  };

  const handleEdit = (data) => {
    dispatch({ type: OPEN_SUBJECT_DIALOG, payload: data });
  };
  ...
  useEffect(() => {
    if (createDone) {
      setOpenSuccess(true);
      dispatch({ type: UNSET_CREATE_SUBJECT_DONE });
    }
  }, [createDone, dispatch]);
  ...
  useEffect(() => {
    if (updateDone) {
      ...
      dispatch({ type: UNSET_UPDATE_SUBJECT_DONE });
    }
  }, [updateDone, dispatch]);
  ...
  useEffect(() => {
    if (deleteDone) {
      ...
      dispatch({ type: UNSET_TEST_DELETE_SUBJECT_DONE });
    }
  }, [deleteDone, dispatch]);
  ...
  const handleSearch = (e) => {
    ...
    if (value) {
      const filteredData = SUBJECT.filter((item) => {
        const courseName = COURSE.find(course => course.id === Number(item.course_id))?.course_name?.toUpperCase() || "";
        const semName = SEM.find(sem => sem.id === Number(item.sem_id))?.sem_name?.toUpperCase() || "";
        const subjectName = item.subject_name?.toUpperCase() || "";
        const description = item.description?.toUpperCase() || "";
  
        return (
          courseName.includes(value) ||
          semName.includes(value) ||
    ...
    <Fragment>
      ...
      >
        <Alert onClose={handleCloseSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> SUBJECT add successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseUpdateSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> SUBJECT update successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseDeleteSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> SUBJECT delete successfully.
          </span>
        </Alert>
      </Snackbar>
