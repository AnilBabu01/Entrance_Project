...
//dialog
import SYLLABUSDialog from "../Dialog/SyllabusDialog";

//redux
import { useDispatch, connect, useSelector } from "react-redux";
import {
  getSYLLABUS,
  blockUnblockSYLLABUS,
  deleteTestcategory,
...
const SYLLABUStable = (props) => {
  ...
  const { SYLLABUS, createDone, updateDone, deleteDone } = useSelector(
  ...
  const blockUnblock = (data) => {
    props.blockUnblockSYLLABUS(data.id);
  };

  const handleOpen = () => {
    dispatch({ type: OPEN_SYLLABUS_DIALOG });
  };

  const handleEdit = (data) => {
    dispatch({ type: OPEN_SYLLABUS_DIALOG, payload: data });
  };
  ...
  useEffect(() => {
    if (createDone) {
      setOpenSuccess(true);
      dispatch({ type: UNSET_CREATE_SYLLABUS_DONE });
    }
  }, [createDone, dispatch]);
  ...
  useEffect(() => {
    if (updateDone) {
      ...
      dispatch({ type: UNSET_UPDATE_SYLLABUS_DONE });
    }
  }, [updateDone, dispatch]);
  ...
  useEffect(() => {
    if (deleteDone) {
      ...
      dispatch({ type: UNSET_TEST_DELETE_SYLLABUS_DONE });
    }
  }, [deleteDone, dispatch]);
    ...
    <Fragment>
      ...
      >
        <Alert onClose={handleCloseSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> SYLLABUS add successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseUpdateSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> SYLLABUS update successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseDeleteSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> SYLLABUS delete successfully.
          </span>
        </Alert>
      </Snackbar>
