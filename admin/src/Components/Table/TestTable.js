...
const TESTtable = (props) => {
  ...
  const { TEST, createDone, updateDone, deleteDone } = useSelector(
    (state) => state.TEST
  );
  ...
  const blockUnblock = (data) => {
    props.blockUnblockTEST(data.id);
  };

  const handleOpen = () => {
    dispatch({ type: OPEN_TEST_DIALOG });
  };

  const handleEdit = (data) => {
    dispatch({ type: OPEN_TEST_DIALOG, payload: data });
  };
  ...
  useEffect(() => {
    if (createDone) {
      setOpenSuccess(true);
      dispatch({ type: UNSET_CREATE_TEST_DONE });
    }
  }, [createDone, dispatch]);
  ...
  useEffect(() => {
    if (updateDone) {
      ...
      dispatch({ type: UNSET_UPDATE_TEST_DONE });
    }
  }, [updateDone, dispatch]);
  ...
  useEffect(() => {
    if (deleteDone) {
      ...
      dispatch({ type: UNSET_TEST_DELETE_TEST_DONE });
    }
  }, [deleteDone, dispatch]);
    ...
    <Fragment>
      ...
      >
        <Alert onClose={handleCloseSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> TEST add successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseUpdateSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> TEST update successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseDeleteSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> TEST delete successfully.
          </span>
        </Alert>
      </Snackbar>
                  ...
                  >
                    <thead>
                      <tr>
                        ...
                        <th>Course</th>
                        <th>Category</th>
                        <th>Physics Question</th>
                        <th>Math Question</th>
                        <th>Chemistry Question</th>
                        <th>English Question</th>
                        <th>Computer Question</th>
                        {/* <th>Block</th> */}
                        <th>Action</th>
                      </tr>
                    </thead>
                                ...
                                <td style={{ verticalAlign: "middle" }}>
                                  {
                                    host.find(
                                      (course) =>
                                        course.id === Number(data.category_id)
                                    )?.category_name
                                  }
                                </td>
                                <td style={{ verticalAlign: "middle" }}>
                                  {JSON.parse(data?.physics_question)?.length}
                                </td>
                                <td style={{ verticalAlign: "middle" }}>
                                  {JSON.parse(data?.math_question)?.length}
                                </td>
                                <td style={{ verticalAlign: "middle" }}>
                                  {JSON.parse(data?.chemistry_question)?.length}
                                </td>
                                <td style={{ verticalAlign: "middle" }}>
                                  {JSON.parse(data?.english_question)?.length}
                                </td>
                                <td style={{ verticalAlign: "middle" }}>
                                  {JSON.parse(data?.computer_question)?.length}
                                </td>
