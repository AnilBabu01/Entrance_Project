...
//dialog
import WHATCLIENTSAYDialog from "../Dialog/WhatClientSayDialog";
...
import {
  getWHATCLIENTSAY,
  blockUnblockWHATCLIENTSAY,
  deleteTestcategory,
} from "../../store/whatclientsay/action";
...
} from "../../store/whatclientsay/types";
...
const WhatClientSayTable = (props) => {
  ...
  const { WHATCLIENTSAY, createDone, updateDone, deleteDone } = useSelector(
    (state) => state.WhatClientSay
  );

  useEffect(() => {
    dispatch(getWHATCLIENTSAY());
  }, [dispatch]);

  useEffect(() => {
    setData(WHATCLIENTSAY);
  }, [WHATCLIENTSAY]);

  const blockUnblock = (data) => {
    props.blockUnblockWHATCLIENTSAY(data.id);
  };

  const handleOpen = () => {
    dispatch({ type: OPEN_WHATCLIENTSAY_DIALOG });
  };

  const handleEdit = (data) => {
    dispatch({ type: OPEN_WHATCLIENTSAY_DIALOG, payload: data });
  };
  ...
  useEffect(() => {
    if (createDone) {
      setOpenSuccess(true);
      dispatch({ type: UNSET_CREATE_WHATCLIENTSAY_DONE });
    }
  }, [createDone, dispatch]);
  ...
  useEffect(() => {
    if (updateDone) {
      ...
      dispatch({ type: UNSET_UPDATE_WHATCLIENTSAY_DONE });
    }
  }, [updateDone, dispatch]);
  ...
  useEffect(() => {
    if (deleteDone) {
      ...
      dispatch({ type: UNSET_TEST_DELETE_WHATCLIENTSAY_DONE });
    }
  }, [deleteDone, dispatch]);
    ...
    <Fragment>
      ...
      >
        <Alert onClose={handleCloseSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> WHATCLIENTSAY add successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseUpdateSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> WHATCLIENTSAY update successfully.
          </span>
        </Alert>
      </Snackbar>
      <Snackbar
      ...
      >
        <Alert onClose={handleCloseDeleteSuccess} severity="success">
          <span style={{ color: "#184d47" }}>
            <b>Success!</b> WHATCLIENTSAY delete successfully.
          </span>
        </Alert>
      </Snackbar>
