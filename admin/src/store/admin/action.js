...
export const login = (email, password, key, package_) => (dispatch) => {
  ...
  axios
    .post("/admin/loginAdmin", { email, password, key, package_ })
    .then((res) => {
      if (res.data.status) {
        dispatch({ type: SET_ADMIN, payload: res.data.data[0].token });
      } else {
        dispatch({ type: SET_LOGIN_ERROR, payload: res.msg });
      }
    })
    .catch((error) => {
...
export const updateProfile = (profileData) => (dispatch) => {
  axios
    .patch("/admin/updateAdminProfile", profileData)
    .then((res) => {
