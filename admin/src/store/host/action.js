...
export const getHostProfile = (hostId) => (dispatch) => {
  axios
    .get(`/host/hostProfile?host_id=${hostId}`)
    .then((res) => {
      debugger;
      dispatch({ type: GET_HOST_PROFILE, payload: res.data.host });
    })
    .catch((error) => console.log(error));
};
...
export const blockUnblockHost = (id) => (dispatch) => {
  apiInstanceFetch
    .get(`/admin/block-note-category/${id}`)
    .then((res) => {
...
export const deleteTestcategory = (id) => (dispatch) => {
  apiInstanceFetch
    .delete(`/admin/test-category/${id}`)
    .then((res) => {

      console.log("delete",res.data );

      dispatch({ type: TEST_DELETE_CATEGORY_DONE, payload: res.data });
    })
    .catch((error) => console.log(error));
};
...
export const createNewHost = (formData) => (dispatch) => {
  axios
    .post("/admin/test-category", formData)
    .then((res) => {
      dispatch({ type: CREATE_NEW_HOST, payload: res.data.data });
...
export const editHost = (formData, id) => (dispatch) => {
  axios
    .put("/admin/test-category", formData)
    .then((res) => {
